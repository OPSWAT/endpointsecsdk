...
void UnzipPackage(const std::string& filePath, const std::string& filename) {
    ...
    // Detect if it's a .zip or .tar file by extension
    if (filePath.size() > 4 && filePath.substr(filePath.size() - 4) == ".zip") {
        std::string cmd = "unzip -o \"" + filePath + "\" -d stage/extract/";
        std::cout << "[DEBUG] Executing: " << cmd << std::endl;
        int result = std::system(cmd.c_str());
        if (result != 0) {
            std::cerr << "[!] Failed to unzip: " << filename << "\n";
        }
    } else if (filePath.size() > 4 && filePath.substr(filePath.size() - 4) == ".tar") {
        ...
        std::cerr << "[!] Unknown file type for extraction: " << filename << "\n";
    }
}
...
void CopyExtractedBinaries(const std::string& sourcePath, const std::string& destPath) {
    try {
        ...
        // Use 'cp -R' for macOS compatibility
        std::string cmd = "cp -R \"" + src + "\"* \"" + dst + "\" 2>/dev/null || :";
